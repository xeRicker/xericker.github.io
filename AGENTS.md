...
- Material Symbols: use only real icon names. A name without a ligature (e.g. `monitoring_off`) renders as overlapping text and blows up the control; `overflow: hidden` on `.material-symbols-rounded` clips it instead of breaking the layout.
- Catalog saves are verified: `saveLocalJson` reads the file back, `saveGithubConfig` compares the content returned by the API, and a failed write keeps the panel dirty with an actionable message instead of looking like success.
- Catalog reads go through the GitHub API first when a token is configured (`fetchRootCatalog`), because GitHub Pages can still serve the previous deployment right after a save. `database/locations.json` also carries `updatedAt`, which the PUNKTY panel compares before overwriting — a catalog changed in another tab asks for confirmation instead of silently reverting work.
