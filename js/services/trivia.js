...
export class TriviaService {
    ...
    generate(data, limit = 10) {
        ...
        // Filtrujemy dni bez utargu dla statystyk finansowych
        const revenueData = data.filter(d => d.total > 0);

        const available = this.generators
            .map(gen => gen.call(this, data, revenueData))
            .filter(Boolean);

        return available.sort(() => 0.5 - Math.random()).slice(0, limit);
    }

    highestRevenueDay(data, revData) {
        if(revData.length === 0) return null;
        const best = revData.reduce((max, d) => d.total > max.total ? d : max, revData[0]);
        return {
            icon: '🏆',
    ...
    locationBattle(data, revData) {
        if(revData.length === 0) return null;
        const osw = revData.reduce((s, d) => s + d.oswiecim, 0);
        const wil = revData.reduce((s, d) => s + d.wilamowice, 0);
    ...
    averageFriday(data, revData) {
        const fridays = revData.filter(d => d.dayOfWeek === 'piątek');
    ...
    cardVsCash(data, revData) {
        if(revData.length === 0) return null;
        const total = revData.reduce((s, d) => s + d.total, 0);
        const card = revData.reduce((s, d) => s + d.cardTotal, 0);
        if (total === 0) return null;
    ...
    drwalKing(data) {
        ...
        data.forEach(d => d.rawReports.forEach(r => {
            if(r.products) {
                let count = 0;
                Object.entries(r.products).forEach(([name, qty]) => {
                    if(name.includes("Drwal")) count += qty;
                });
                if(count > maxDrwal) { maxDrwal = count; bestDate = d.dateStr; }
            }
        }));

        if(maxDrwal === 0) return null;

        return {
            icon: '🍔',
            title: 'Król Drwala',
            text: `Rekord sprzedaży Drwala padł <strong>${bestDate}</strong>. Sprzedano wtedy łącznie <strong>${maxDrwal} sztuk</strong>!`
        };
    }
    ...
    earlyBird(data) {
        ...
        data.forEach(d => d.rawReports.forEach(r => {
            if(r.employees) Object.entries(r.employees).forEach(([name, time]) => {
                ...
                if(start < earliest && start > "06:00") { earliest = start; empName = name; }
            });
        }));
        ...
        return {
            icon: '🌅',
            title: 'Ranny Ptaszek',
            text: `Najwcześniej pracę zaczął(a) <strong>${empName}</strong>. Była to godzina <strong>${earliest}</strong>.`
        };
    }
}
